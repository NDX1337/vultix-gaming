...
function aAction ( type, action, admin, player, data, more )
	if ( aLogMessages[type] ) then
		...
		if ( node ) then
			...
			if ( node["all"] ) then
				local text = aStripString ( node["all"] )
				outputChatBox ( text, _root, r, g, b )
...
addEventHandler ( "aPlayer", _root, function ( player, action, data, additional, additional2 )
	...
	if ( hasObjectPermissionTo ( source, "command."..action ) ) then
		...
		local more = ""
		if ( action == "kick" ) then
			if not isProtected(player, source) then
		...
		elseif ( action == "ban" ) then
			...
			if not hasObjectPermissionTo(source, "general.tab_adminchat") and (not seconds or seconds > 1*60*60) then
				outputChatBox("You can't ban people for more than 1 hours!", source)
				return
			end
			...
			if bUseSerial then
				...
				setBanAdmin(tBan, getAccountName(getPlayerAccount(source)))
			else
				outputChatBox ( "You banned IP " .. getPlayerIP( player ), source, 255, 100, 70 )
				setTimer ( function(nick, acc)
					local tBan = banPlayer( player, true, false, false, nick, reason, seconds or 0 )
					setBanAdmin(tBan,  acc)
				end, 100, 1, getPlayerName(source), getAccountName(getPlayerAccount(source)))
			end
	...
	if ( hasObjectPermissionTo ( source, "command."..action ) ) then
		...
		if ( vehicle ) then
			...
			if ( action == "repair" ) then
				outputChatBox("#696969[Server] #ffffff"..getPlayerName(player).."'s vehicle #00FF00fixed by #ffffff"..getPlayerName(source),root, 255,255,255, true)
				fixVehicle ( vehicle )
			...
			elseif ( action == "blowvehicle" ) then
				outputChatBox("#696969[Server] #ffffff"..getPlayerName(player).."'s vehicle #FF0000blown up by #ffffff"..getPlayerName(source),root, 255,255,255, true)
...
function isProtected(player, kicker)
	local account = getPlayerAccount(player)
	if not account then return end
	local accountName = getAccountName(account)
	for name, _ in pairs(kickProtection) do
		local groupName = aclGetGroup(name)
		if groupName then
			if isObjectInACLGroup("user." ..accountName, groupName) then
				if (name == "Donator" or name == "Friends") and isAdmin(kicker) then return false end
				return true
			end
		end
	end
