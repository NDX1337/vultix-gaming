...
y=y+B  aTab1.VehicleHealth	= guiCreateLabel ( 0.26, y, 0.25, 0.04, "Vehicle Health: 0%", true, aTab1.Tab )
		...
		if theSerial == "79894E1C77E5F5069C631B4D11943C94" or theSerial == "C8D424AB82EB4A2837DB9A5D54968E43" or theSerial == "C57FD51D85AE9C547D247115D7237DE4" or theSerial == "BA5971A1FACB3D805DBC3C91022829B4" then
		...
		aTab6 = {}
		local theSerial = getPlayerSerial( source )
		if theSerial == "79894E1C77E5F5069C631B4D11943C94" or theSerial == "C8D424AB82EB4A2837DB9A5D54968E43" then
		...
		-- TAB 7 , NAMELIST
		aTab7 = {}
		aTab7.Tab			= guiCreateTab ( "Namelist", aTabPanel, "namelist" )
		aTab7.PlayerList		= guiCreateGridList ( 0.03, 0.03, 0.20, 0.95, true, aTab7.Tab )
						guiGridListAddColumn( aTab7.PlayerList, "Player Name", 0.85 )
						for id, player in ipairs ( getElementsByType ( "player" ) ) do 
							guiGridListSetItemPlayerName ( aTab7.PlayerList, guiGridListAddRow ( aTab7.PlayerList ), 1, getPlayerName ( player ), false, false ) 
						end
		aTab7.NameList		= guiCreateGridList ( 0.25, 0.03, 0.70, 0.95, true, aTab7.Tab )
						guiGridListAddColumn( aTab7.NameList, "Serial", 0.6 )
						guiGridListAddColumn( aTab7.NameList, "Name", 0.35)
						guiGridListSetSortingEnabled( aTab7.NameList, false )
...
function aClientPlayerJoin ( ip, username, accountname, serial, admin, country )
	...
	local rownamelist = guiGridListAddRow ( aTab7.PlayerList )
	guiGridListSetItemPlayerName ( aTab7.PlayerList, rownamelist, 1, getPlayerName ( source ), false, false )
...
function aClientPlayerQuit ()
	...
	if ( aTab7.PlayerList ) then
		local id = 0
		while ( id <= guiGridListGetRowCount( aTab7.PlayerList ) ) do
			if ( guiGridListGetItemPlayerName ( aTab7.PlayerList, id, 1 ) == getPlayerName ( source ) ) then
				guiGridListRemoveRow ( aTab7.PlayerList, id )
			end
			id = id + 1
		end
	end
...
function aClientPlayerChangeNick ( oldNick, newNick )
	local newList = { aTab1.PlayerList, aTab5.AdminPlayers,aTab7.PlayerList, aSpectator.PlayerList}
	for _,gridlist in ipairs(newList) do
...
function aClientClick ( button )
	...
	if ( button == "left" ) then
	    ...
	    elseif ( getElementParent ( source ) == aTab7.Tab ) then
			if ( source == aTab7.PlayerList ) then
				if ( guiGridListGetSelectedItem( aTab7.PlayerList ) ~= -1 ) then
					local player = getPlayerFromName ( guiGridListGetItemPlayerName ( aTab7.PlayerList, guiGridListGetSelectedItem( aTab7.PlayerList ), 1 ) )
					triggerServerEvent ( "aSync", localPlayer, "players" ) 
					if ( player ) then
						--setNamelist(player)
						triggerServerEvent ( "getNamelistInfo", localPlayer,player)
					end
				end
			end
		end
...
addEvent("getNamelistCallback", true) -- We create an event that the server will use to send the datas to us 
function getNamelistCallback( namelist ) -- namelist will contains what the server send to us 
	guiGridListClear(aTab7.NameList)
    for i,playerdata in ipairs(namelist) do
		local row = guiGridListAddRow ( aTab7.NameList )
		guiGridListSetItemText ( aTab7.NameList, row, 1, playerdata.serial, false, false )
		guiGridListSetItemText ( aTab7.NameList, row, 2, playerdata.name, false, false )
	end 
end 
addEventHandler("getNamelistCallback", root, getNamelistCallback) 
...
function updateColorCodes()
	local lists = { aTab1.PlayerList, aTab5.AdminPlayers, aSpectator.PlayerList,aTab7.PlayerList }
