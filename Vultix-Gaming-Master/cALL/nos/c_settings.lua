...
function changeNosGaugePosition(_, btnState, x, y)
	if btnState == "down" then
		g_Settings.GaugePosition[1] = x + 320
		g_Settings.GaugePosition[2] = y + 1000
	end
...
function consoleSetNosFiringStyle(commandName, nosControl)
	...
	if nosControl ~= "hybrid" and nosControl ~= "HYBRID" and nosControl ~= "nfs" and nosControl ~= "NFS" and nosControl ~= "normal" and nosControl ~= "NORMAL" then
		if commandName == "nos" then
			outputChatBox("#e60000[HELP] #ffffffTo change NITRO use: /nos NORMAL, NFS, HYBRID",255,255,255,true)
		else
			outputChatBox("#e60000[HELP] #ffffffTo change NITRO use: /nos NORMAL, NFS, HYBRID",255,255,255,true)
		end
		return
	...
	if g_Settings.ControlStyle ~= nosControl then
		...
		if saveSettingsToFile() then
			outputChatBox("#0066ff[NOS]: #ffffffyou set #00ff00"..tostring(settingName).." #ffffffmode",255,255,255,true)
		end
	end
end
addCommandHandler("nos", consoleSetNosFiringStyle)

function getVehicleNitroStyle()
    return g_Settings.ControlStyle
end
...
function maybeDisplaySpecialMessage()
	...
	if counter==1 or counter==2 or counter==6 then
		--outputChatBox ( "F3 Basarak Nos Stilinizi Deðiþtirebilirsiniz." )
		--outputChatBox ( "F3 Basarak Nos Stilinizi Deðiþtirebilirsiniz." )
	end
