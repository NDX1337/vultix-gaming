...
function RaceMode.endMap()
    if stateAllowsPostFinish() then
        ...
        local text = g_GameOptions.randommaps and 'Next round starts in:' or 'Next round starts in:'
        Countdown.create(5, RaceMode.startNextMapSelect, text, 255, 255, 255, 0.6, 2.0 ):start()
...
function RaceMode:onPlayerWasted(player)
	...
	if RaceMode.getMapOption('respawn') == 'timelimit' and not RaceMode.isPlayerFinished(source) then
        ...
        if self:getTimeRemaining() - respawnTime > 3000 then
            Countdown.create(respawnTime/1000, restorePlayer, 'Respawning in:', 255, 255, 255, 0.25, 2.0, true, self.id, player):start(player)
        end
