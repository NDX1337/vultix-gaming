...
addEventHandler('onClientResourceStart', g_ResRoot,
	function()
		...
		g_dxGUI = {
			...
			checkpoint = dxText:create('0/0', screenWidth - 15, screenHeight - 54, false, 'default-bold', 1.2, 'right'),
			--timepassed = dxText:create('0:00:00', screenWidth - 15, screenHeight - 25, false, 'default-bold', 0, 'center'),
			--mapdisplay = dxText:create('No map started..', 2, screenHeight - dxGetFontHeight(1.2, 'default-bold')/2, false, 'default-bold', 1.1, 'left')
		}
		--fps = dxText:create('FPS: 45', 2, screenHeight - dxGetFontHeight(3.5, 'bankgothic')/2, false, 'bankgothic', 0.6, 'left')
		--ping = dxText:create('Ping: 0', 2, screenHeight - dxGetFontHeight(2.5, 'bankgothic')/2, false, 'bankgothic', 0.6, 'left')
		--fps:type('stroke', 1, 0, 0, 0, 175)
		--ping:type('stroke', 1, 0, 0, 0, 175)
		--g_dxGUI.mapdisplay:type('stroke', 1, 0, 0, 0, 175)
		...
		--g_dxGUI.timepassed:type('stroke', 1, 0, 0, 0, 175)
		g_GUI = {
			--timeleftbg = guiCreateStaticImage(screenWidth/2-108/2, 0, 108, 24, 'img/timelef.png', false, nil),
			--timeleft = guiCreateLabel(screenWidth/2-108/2, 2, 108, 30, '', false),
			--healthbar = FancyProgress.create(250, 1000, 'img/progress_health_bg.png', -65, 49, 123, 30, 'img/progress_health.png', 7, 7, 109, 16),
			--speedbar = FancyProgress.create(11000, 1151, 'img/map.png', 5, -2.5, 21, 21, '', 35, 8, 108, 15),
			--speedbar = FancyProgress.create(250, 1000, 'img/icon.png', -5, -15, 30, 30, '', 35, 8, 108, 15),
		}
...
function TitleScreen.init()
	...
	g_dxGUI['titleText1'] = dxText:create('', 0, screenHeight-107, false, 'default-bold-small', 0, 'left' )
	g_dxGUI['titleText2'] = dxText:create('', 90, screenHeight-107, false, 'default-bold-small', 0, 'left' )
...
function TravelScreen.init()
    local screenWidth, screenHeight = guiGetScreenSize()
    g_GUI['travelImage']   = guiCreateStaticImage(screenWidth/2-960, screenHeight/2-60, 1920, 120, 'img/travelling.png', false, nil)
	g_dxGUI['travelText1'] = dxText:create('Next map will be started..', screenWidth/2, screenHeight/2-130, false, 'default-bold', 1.3, 'center' )
	g_dxGUI['travelText2'] = dxText:create('', screenWidth/2, screenHeight/2-100, false, 'default-bold', 1.1, 'center' )
	g_dxGUI['travelText3'] = dxText:create('', screenWidth/2, screenHeight/2-70, false, 'default-bold', 1.1, 'center' )
    g_dxGUI['travelText1']:color(255,255,255)
...
function launchRace(duration)
	...
	if type(duration) == 'number' then
		g_Duration = duration
		addEventHandler("onClientRender", root, renderShit,false,"low");
	end
...
function Spectate._start()
	...
	local screenWidth, screenHeight = guiGetScreenSize()
	g_GUI.speclabel = guiCreateLabel(screenWidth/2 - 100, screenHeight - 210, 200, 70, '', false)
	Spectate.updateGuiFadedOut()
	guiLabelSetHorizontalAlign(g_GUI.speclabel, 'center')
	    guiSetFont(g_GUI.speclabel, "default-bold-small")
	...
	Spectate.specTimer = setTimer ( function()
	if Spectate.target then
	local targetFPS = getElementData(Spectate.target, "fps")
	local targetPing = getPlayerPing(Spectate.target)
	dxDrawText ( "FPS:"..targetFPS.." Ping:"..targetPing, screenWidth/2-42, screenHeight-180, 400, 400, tocolor ( 255, 255, 255, 255 ), 0.98, "default-bold" )
	end
	end, 0, 0)
end
...
function unloadAll()
	...
	g_Vehicle = nil
	removeEventHandler("onClientRender", root, renderShit,"low");
...
--function()
	--fps
	--local fpstext = tostring("Fps: "..( getElementData(getLocalPlayer(), 'fps') or 0 ))
	--fps:text(fpstext)
	--ping
	--pingtext = tostring("Ping: "..getPlayerPing(localPlayer))
	--ping:text(pingtext)
	--end
--,1000,0)
...
local generalAlpha = 255
local hudEnabled = true
--CORE'S STUFF MK
function renderShit()
	if not hudEnabled then
		generalAlpha = math.max(generalAlpha - 15, 0)
	else
		generalAlpha = getKeyState("tab") and math.max(generalAlpha - 15, 0) or math.min(generalAlpha + 15, 255)
	end
	if generalAlpha > 0 then
...
local HP = {}
	...
	HP.posY = SY / 13
	HP.width = SX / 9
	HP.fontHeight = dxGetFontHeight(1, "default-bold")
	HP.scale = 1.2
	HP.height = HP.fontHeight * HP.scale
	HP.healthwidth = 5.29
	HP.healthStrposX = HP.posX * 1.140
	HP.healthStrposY = HP.posY * 1
	HP.healthStrWidth = dxGetTextWidth("[ %100 ]", 1, "default-bold")

local mapDisplay = {}
	mapDisplay.offsetX = 5
	mapDisplay.offsetY = 5
	mapDisplay.scale = 1.75
	mapDisplay.fontSize = 1
	mapDisplay.font = "default-bold"
	mapDisplay.fontHeight = dxGetFontHeight(mapDisplay.fontSize, mapDisplay.font)
	mapDisplay.height = mapDisplay.fontHeight * mapDisplay.scale
	mapDisplay.posX = mapDisplay.offsetX
	mapDisplay.posY = SY - mapDisplay.height - mapDisplay.offsetY
	mapDisplay.width = 0
	
	local veh = getPedOccupiedVehicle(localPlayer)
	if veh then
	local healthlength = getVehicleHealth(veh)
	dxDrawRectangle(HP.posX, HP.posY, HP.width/100*healthlength, HP.height, tocolor(255,0,0,math.min(255, generalAlpha)));
	dxDrawRectangle(HP.posX, HP.posY, HP.width, HP.height, tocolor(0,0,0,math.min(180, generalAlpha)));
	dxDrawRectangle(HP.posX*1.138, HP.posY, HP.healthStrWidth, HP.height, tocolor(0,0,0,math.min(180, generalAlpha)));
	
	dxDrawText("[ %"..healthlength.." ]", HP.healthStrposX, HP.healthStrposY, HP.healthStrWidth, HP.height, tocolor(255,255,255,math.min(255, generalAlpha)), 1, "default-bold");
	dxDrawRectangleGradiente(HP.posX, HP.posY, HP.width/100*healthlength, HP.height, 158,14,14,math.min(255, generalAlpha), 'right');
	end
	
	local mapName = g_MapInfo.name
	local edgeSize = mapDisplay.height/2
	local iconSize = mapDisplay.height * 0.5
	local iconCenterTop = (mapDisplay.height - iconSize)/2
	local mapNameWidth = dxGetTextWidth(mapName, mapDisplay.fontSize, mapDisplay.font) + iconSize * 100.25
	if mapNameWidth <= 10 then
		mapNameWidth = 0
	end
	mapDisplay.width = mapDisplay.width < mapNameWidth and math.min(mapDisplay.width + 5, mapNameWidth) or math.max(mapDisplay.width - 5, mapNameWidth)
	local offsetY = mapDisplay.posY
	local width = dxGetTextWidth("Map: "..mapName.."",mapDisplay.fontSize, mapDisplay.font,false)
	dxDrawRectangle(mapDisplay.posX + edgeSize, offsetY, width, mapDisplay.height, tocolor(0, 0, 0, math.min(180, generalAlpha)), false)
	dxDrawImageSection(mapDisplay.posX, offsetY, edgeSize, mapDisplay.height, 1, 1, 15, 30, "img/edge.png", 0, 0, 0, tocolor(0, 0, 0, math.min(180, generalAlpha)), false)
    dxDrawImageSection(mapDisplay.posX + width + edgeSize, offsetY, edgeSize, mapDisplay.height, 16, 1, 15, 30, "img/edge.png", 0, 0, 0, tocolor(0, 0, 0, math.min(180, generalAlpha)), false)
	dxDrawImageSection(mapDisplay.posX + iconCenterTop, offsetY + iconCenterTop, iconSize, iconSize, 1, 1, 512, 512, "img/map.png", 0, 0, 0, tocolor(255, 255, 255, math.min(225, generalAlpha)), false)
	dxDrawText(mapName.."", mapDisplay.posX + iconCenterTop * 2 + iconSize, offsetY * 2, mapDisplay.width, mapDisplay.height, tocolor(255, 255, 255, math.min(225, generalAlpha)), mapDisplay.fontSize, mapDisplay.font,"left","center",false,false,false,true)

--TIMEDISPLAY
	local timeDisplay = {}
	timeDisplay.timeLeft = msToTimeStr(timeLeft > 0 and timeLeft or 0)
	timeDisplay.offsetX = 5
	timeDisplay.offsetY = 6
	timeDisplay.scale = 1.75
	timeDisplay.fontSize = 1
	timeDisplay.font = "default-bold"
	timeDisplay.fontHeight = dxGetFontHeight(timeDisplay.fontSize, timeDisplay.font)
	timeDisplay.height = timeDisplay.fontHeight * timeDisplay.scale
	timeDisplay.width = dxGetTextWidth(timeDisplay.timeLeft, timeDisplay.fontSize, timeDisplay.font) * 1.5
	timeDisplay.posX = (SX-timeDisplay.width)/2
	timeDisplay.posY = timeDisplay.offsetY
	timeDisplay.rotation = 0
	offsetY = timeDisplay.posY
	edgeSize = timeDisplay.height/2
	iconSize = timeDisplay.height * 0.5
	iconCenterTop = (timeDisplay.height - iconSize)/2
	dxDrawRectangle(timeDisplay.posX - edgeSize, offsetY, timeDisplay.width, timeDisplay.height, tocolor(0, 0, 0, math.min(180, generalAlpha)), false)
	dxDrawImageSection(timeDisplay.posX - edgeSize * 2, offsetY, edgeSize, timeDisplay.height, 1, 1, 15, 30, "img/edge.png", 0, 0, 0, tocolor(0, 0, 0, math.min(180, generalAlpha)), false)
	dxDrawImageSection(timeDisplay.posX - edgeSize * 2, offsetY, timeDisplay.height, timeDisplay.height, 1, 1, 30, 30, "img/edge.png", 0, 0, 0, tocolor(0, 0, 0, math.min(155, generalAlpha)), false)
	dxDrawImageSection(timeDisplay.posX + timeDisplay.width - edgeSize, offsetY, edgeSize, timeDisplay.height, 16, 1, 15, 30, "img/edge.png", 0, 0, 0, tocolor(0, 0, 0, math.min(180, generalAlpha)), false)
	dxDrawImageSection(timeDisplay.posX - edgeSize * 2 + iconCenterTop, offsetY + iconCenterTop, iconSize, iconSize, 1, 1, 30, 30, "img/time.png", 0, 0, 0, tocolor(171, 205, 239, math.min(225, generalAlpha)), false)
	dxDrawText(timeDisplay.timeLeft, timeDisplay.posX - edgeSize * 2 + iconCenterTop * 3 + iconSize, offsetY, timeDisplay.width, timeDisplay.height, tocolor(255, 255, 255, math.min(225, generalAlpha)), mapDisplay.fontSize, mapDisplay.font, "left", "bottom")

	--[[dxDrawRectangle(timeDisplay.posX - edgeSize, offsetY, timeDisplay.width, timeDisplay.height, tocolor(0, 0, 0, math.min(180, generalAlpha)), false)
	dxDrawImageSection(timeDisplay.posX - edgeSize * 2, offsetY, edgeSize, timeDisplay.height, 1, 1, 15, 30, "img/edge.png", 0, 0, 0, tocolor(0, 0, 0, math.min(180, generalAlpha)), false)
	dxDrawImageSection(timeDisplay.posX + timeDisplay.width - edgeSize, offsetY, edgeSize, timeDisplay.height, 16, 1, 15, 30, "img/edge.png", 0, 0, 0, tocolor(0, 0, 0, math.min(180, generalAlpha)), false)
	dxDrawImageSection(timeDisplay.posX - edgeSize * 2 + iconCenterTop, offsetY + iconCenterTop, iconSize, iconSize, 1, 1, 30, 30, "img/timepassed.png", timeDisplay.rotation, 0, 0, tocolor(171, 205, 239, math.min(225, generalAlpha)), false)
	--dxDrawImage(timeDisplay.posX*1.04 , offsetY-iconCenterTop, iconSize*4, iconSize*4,"img/logo.png", 0, 0,0, tocolor(255,255,255,math.min(255, generalAlpha)))
	dxDrawText(timeDisplay.timeLeft, timeDisplay.posX - edgeSize * 2 + iconCenterTop * 3 - iconSize, offsetY, timeDisplay.width, timeDisplay.height, tocolor(255, 255, 255, math.min(255, generalAlpha)), mapDisplay.fontSize, mapDisplay.font, "left", "bottom")	
	dxDrawText(timeDisplay.timePassed, timeDisplay.posX - edgeSize * 2 + iconCenterTop * 13 + iconSize, offsetY, timeDisplay.width, timeDisplay.height, tocolor(255, 255, 255, math.min(255, generalAlpha)), mapDisplay.fontSize, mapDisplay.font, "left", "bottom")
	]]
	end
end	
...
function dxDrawRectangleGradiente(x,y,w,h,r,g,b,a,direction)
    if (not x or not y or not w or not h or not r or not g or not b or not a or not direction) then
	    return outputDebugString("dxDrawRectangleGradiente: Required arguments are missing.", 1);
	end
	...
	elseif (direction == "up") then
	    ...
	    return outputDebugString("dxDrawRectangleGradiente: Invalid direction", 1);
	end
end