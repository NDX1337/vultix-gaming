...
addCommandHandler('redo',
	function( player, command, value )
		if isPlayerInACLGroup(player, g_GameOptions.admingroup) then
			local currentMap = exports.mapmanager:getRunningGamemodeMap()
			if currentMap then
				outputChatBox('Map restarted by ' .. getPlayerName(player), g_Root, 170, 8, 68)
	...
	function( player, command, value )
		if isPlayerInACLGroup(player, g_GameOptions.admingroup) then
			if not stateAllowsRandomMapVote() or g_CurrentRaceMode:getTimeRemaining() < 1000 then
				...
				outputChatBox('Current map ' .. choice[math.random( 1, #choice )] .. ' by ' .. getPlayerName(player), g_Root, 170, 8, 68)
				startRandomMap()
			end
		end
	end
...
addCommandHandler('nextmap',
	function( player, command, ... )
		...
		g_ForcedNextMap = map
		outputChatBox('Next map set to ' .. getMapName( g_ForcedNextMap ) .. ' by ' .. getPlayerName( player ), g_Root, 170, 8, 68)
	end
)
