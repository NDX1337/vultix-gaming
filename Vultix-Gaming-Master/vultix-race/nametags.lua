...
function showPing()
	pingMode = not(pingMode)
end
addCommandHandler('showping',showPing) 
		...
		for player in pairs(nametags) do 
			while true do
				...
				if pdistance <= NAMETAG_DISTANCE then
					...
					-- Original line ! DONT DELETE THIS
					...
					if playerPing <= 69 then
						r2, g2 = 0, 255
					elseif playerPing > 99 then
						r2 = 255
						g2 = 0
					elseif playerPing > 70 then
						r2 = 255
						g2 = 255
					end		
					if pingMode then
						dxDrawText (""..string.format("#%.2X%.2X%.2X", r, g, b)..getPlayerName(player)..string.format("#%.2X%.2X%.2X", r2, g2, 0).." ("..playerPing..")", sx, sy - offset, sx, sy - offset, tocolor(r,g,b,textalpha), textscale*NAMETAG_TEXTSIZE, "default", "center", "bottom", false, false, false ,true)
					else
						dxDrawText ( getPlayerName(player), sx, sy - offset, sx, sy - offset, tocolor(r,g,b,textalpha), textscale*NAMETAG_TEXTSIZE, "default", "center", "bottom", false, false, false )
					end
					--dxDrawText ( getPlayerName(player), sx, sy - offset, sx, sy - offset, tocolor(r,g,b,textalpha), textscale*NAMETAG_TEXTSIZE, "default", "center", "bottom", false, false, false )
