...
addEventHandler('onPlayerJoin', g_Root,
	function()
		outputConsole ( 'Race version ' .. getBuildString(), source, 0,255,0 )
...
g_RaceStartCountdown = Countdown.create(2, launchRace)
g_RaceStartCountdown:useImages('img/countdown_%d.png', 128, 128)
...
addEventHandler('onClientRequestSpectate', g_Root,
	function(enable)
		...
		-- Disable in non-RS maps
		if not g_MapInfo.name:find("%[RS%]") then return end
...
addCommandHandler('restartracemode',
	function(player)
		...
		outputChatBox('Race restarted by ' .. getPlayerName(player), g_Root, 0,255,0)
		exports.mapmanager:changeGamemode( getResourceFromName('race') )
	end
)
